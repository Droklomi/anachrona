...
export default function MythologiePage() {
  ...
  return (
    <div style={{ background: '#04020E', minHeight: '100vh' }}>
      ...
      <nav className={`mn ${scrolled ? 'scrolled' : ''}`}>
        <button className="mn-back" onClick={() => router.push('/')}>← Histoire</button>
        <span className="mn-title">Anachrona Mythologie</span>
        <button className="mn-cta" onClick={() => router.push('/login')}>
      ...
      {!isPremium && (
        <div className="mp-cta">
          <div className="mp-box">
            ...
            <button className="mp-btn" onClick={() => router.push('/login')}>Obtenir le Pass Mythologie</button>
          </div>
        </div>
      )}
      ...
      <div className="mfoot">
        ...
        <span style={{ cursor:'pointer', opacity:.6 }} onClick={() => router.push('/')}>
